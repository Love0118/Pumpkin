...
/// Runtime behaviour for vanilla's six-sided, waterloggable glow lichen.
...
impl BlockBehaviour for GlowLichenBlock {
    ...
    fn get_state_for_neighbor_update<'a>(
        ...
        Box::pin(async move {
            ...
            // A neighbour update invalidates only the lichen face that touches it.
    ...
    fn use_with_item<'a>(
        ...
        Box::pin(async move {
            if args.item_stack.item.id != Item::GLOW_LICHEN.id {
            ...
            args.world
                .set_block_state(
                    args.position,
                    state_with_added_face(args.block, Some(current_state.id), direction, false),
...
mod tests {
    ...
    #[test]
    fn glow_lichen_is_registered_in_the_default_block_registry() {
        let registry = crate::block::registry::default_registry();

        assert!(registry.get_pumpkin_block(BlockId::GLOW_LICHEN).is_some());
    }
}
