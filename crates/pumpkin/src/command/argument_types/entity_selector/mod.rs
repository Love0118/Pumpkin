...
impl EntitySelector {
    ...
    async fn add_players_from_world(
        ...
        let players = world.players.load().iter().cloned().collect::<Vec<_>>();
...
async fn matches_async_nbt_snapshot(
    expected: &NbtCompound,
    invert: bool,
    snapshot: impl Future<Output = NbtCompound>,
) -> bool {
    matches_nbt_compound(expected, &snapshot.await) ^ invert
}
        ...
        Box::pin(async move {
            match self {
                ...
                Self::Tag(expected_tag, invert) => {
                    let has_tag = entity.get_entity().has_scoreboard_tag(expected_tag).await;
                    has_tag ^ invert
                }
                ...
                Self::Nbt(expected_nbt, invert) => {
                    matches_async_nbt_snapshot(expected_nbt, *invert, async {
...
#[cfg(test)]
mod tests {
    use super::matches_async_nbt_snapshot;
    use pumpkin_nbt::compound::NbtCompound;
    use std::time::Duration;
    use tokio::time::timeout;

    async fn yielded_snapshot() -> NbtCompound {
        tokio::task::yield_now().await;
        let mut actual = NbtCompound::new();
        actual.put_int("Health", 20);
        actual
    }

    async fn assert_async_snapshot_predicate_completes() {
        let mut expected = NbtCompound::new();
        expected.put_int("Health", 20);
        let matched = timeout(
            Duration::from_secs(1),
            matches_async_nbt_snapshot(&expected, false, yielded_snapshot()),
        )
        .await
        .expect("native async NBT predicate should not starve the runtime");
        assert!(matched);
    }

    #[tokio::test(flavor = "current_thread")]
    async fn nbt_predicate_completes_on_current_thread_runtime() {
        assert_async_snapshot_predicate_completes().await;
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn nbt_predicate_completes_on_multi_thread_runtime() {
        assert_async_snapshot_predicate_completes().await;
    }
}
